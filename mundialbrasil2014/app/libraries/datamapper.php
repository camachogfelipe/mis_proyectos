...
class DataMapper implements IteratorAggregate {
	...
	protected static $model_paths = array();
        ...
        //variable insertada por elbert tous 
        public $_type_join = 'LEFT OUTER'; 
	...
	public function count($exclude_ids = NULL, $column = NULL, $related_id = NULL)
	{
		...
		if ( ! empty($this->parent))
		{
			...
			if( ! empty($this->db->ar_where))
			{
				...
				{
					$this->db->join($this->table, $this->table . '.id = ' . $relationship_table . '.' . $this_model.'_id', $this->_type_join);
				}
	...
	public function _add_related_table($object, $related_field = '', $id_only = FALSE, $db = NULL, &$query_related = NULL, $name_prepend = '', $this_table = NULL)
	{
		...
		if ($relationship_table == $this->table && in_array($other_column, $this->fields))
		{
			...
			{
				$db->join($object->table . ' ' .$object_as, $object_as . '.id = ' . $this_table . '.' . $other_column, $this->_type_join);
		...
		else if ($relationship_table == $object->table && in_array($this_column, $object->fields))
		{
			...
			{
				$db->join($object->table . ' ' .$object_as, $this_table . '.id = ' . $object_as . '.' . $this_column, $this->_type_join);
				$query_related[] = $object_as;
			}
			if($id_only)
			{
		...
		else
		{
			...
			{
				$db->join($relationship_table . ' ' . $relationship_as, $this_table . '.id = ' . $relationship_as . '.' . $this_column, $this->_type_join);
			...
			{
				// Add join if not already included
				$db->join($object->table . ' ' . $object_as, $object_as . '.id = ' . $relationship_as . '.' . $other_column, $this->_type_join);
	 ...
	 */
	public function join_related($related_field, $fields = NULL, $append_name = TRUE, $type = 'LEFT OUTER')
	{
                $this->_type_join = $type;                                         
