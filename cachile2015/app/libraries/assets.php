...
class Assets {
    ...
    public static $enable_yuicompressor = false;  // Enable YUICOMpressor
    ...
    // Modular config #rigobcastro
            public static $is_modular = true, $module = null;
    // Flags
    ...
    private static $_yuicompressor_loaded = false;
     ...
     * @return string
     */
    public static function css($files = null, $attributes = null, $module = null, $is_modular = true) {
        self::init();
        self::$module = $module;
        ...
        if (self::$_enable_benchmark)
            self::$_ci->benchmark->mark("Assets::css()_start");
        ...
        // End benchmark
        if (self::$_enable_benchmark)
            self::$_ci->benchmark->mark("Assets::css()_end");
    ...
    public static function css_group($group = null, $files = null, $attributes = null) {
        self::$module = $group;
        $group = underscore_special($group);
        self::$group = $group;
        ...
        // Start benchmark
        if (self::$_enable_benchmark)
            self::$_ci->benchmark->mark("Assets::css_group(" . $group . ")_start");
        ...
        if (self::$_cache_info and self::$freeze) {
            ...
            self::_process('css', $group);
        }

        // End benchmark
        if (self::$_enable_benchmark)
            self::$_ci->benchmark->mark("Assets::css_group(" . $group . ")_end");
     ...
     * @return string
     */
    public static function js($files = null, $module = null, $is_modular = true) {
        self::$group = null;
        self::$module = $module;
        self::$is_modular = $is_modular;
        ...
        // Start benchmark
        if (self::$_enable_benchmark)
            self::$_ci->benchmark->mark("Assets::js()_start");
        ...
        // End benchmark
        if (self::$_enable_benchmark)
            self::$_ci->benchmark->mark("Assets::js()_end");
    ...
    public static function cdn($assets = null, $echo = true) {
        if ($assets) {
            // Start benchmark
            if (self::$_enable_benchmark)
                self::$_ci->benchmark->mark("Assets::cdn()_start");
            ...
            foreach ($assets as $asset) {
                ...
                if (is_array($asset)) {
                    $cdn_asset = $ci->config->item($asset[0], 'assets_cdn');
                    if ($cdn_asset)
                        $version = $asset[1];
                }
                else {
                    $cdn_asset = $ci->config->item($asset, 'assets_cdn');
                    if ($cdn_asset)
                        $version = $cdn_asset['default_version'];
                }
            ...
            // End benchmark
            if (self::$_enable_benchmark)
                self::$_ci->benchmark->mark("Assets::cdn()_end");
    ...
    public static function conditional($condition = null, $string = null) {
        if ($condition and $string) {
            // Start benchmark
            if (self::$_enable_benchmark)
                self::$_ci->benchmark->mark("Assets::conditional()_start");
            ...
            // End benchmark
            if (self::$_enable_benchmark)
                self::$_ci->benchmark->mark("Assets::conditional()_end");
        }
    }
    ...
    private static function _add_assets($assets = null, $group = null, $type = null, $attributes = null) {
        ...
        // Start benchmark
        if (self::$_enable_benchmark)
            self::$_ci->benchmark->mark("Assets::add-assets()_start");
        ...
        if (isset(self::$_cache_info->{$type}->{$group}->file_list))
            self::$_assets[$type][$group]['file_list'] = self::$_cache_info->{$type}->{$group}->file_list;
        else
            self::$_assets[$type][$group]['file_list'] = array();
        ...
        foreach ($assets as $asset) {
            ...
            // Get extension for work with this on -_type
            $pathinfo = (object) pathinfo($asset);
            $_type = $pathinfo->extension;

            // Add file
            $tmp_asset = array('file' => $asset);

            // If isn't modular
            if (false === self::$is_modular) {
                // Determine path
                if ($type === 'css')
                    $file_path = reduce_double_slashes(self::$css_path . '/' . $asset);
                elseif ($type === 'js')
                    $file_path = reduce_double_slashes(self::$js_path . '/' . $asset);
            } else {
                // Obtener path segun el info si es modulo o no
                $ci = get_instance();
                if (empty(self::$module)) {
                    $module = $ci->router->class;
                    $reflector = new ReflectionClass($module);
                    $fn = $reflector->getFileName();
                    $pathinfo = (object) pathinfo($fn);
                    $file_path = dirname($pathinfo->dirname);
                } else {
                    $file_path = FCPATH . APPPATH . 'modules' . DS . self::$module;
                }

                $file_path .= DS . 'assets' . DS . $_type . DS . $asset;
            }
        ...
        // End benchmark
        if (self::$_enable_benchmark)
            self::$_ci->benchmark->mark("Assets::add-assets()_end");
    }
    ...
    private static function _process($type = null, $group = null) {
        ...
        if ($group !== self::$default_group[$type])
            $file_prefix = $group . '-' . md5($last_modified);
        else
            $file_prefix = 'cms-' . md5($last_modified);
        ...
        if (!$file_exists or !$last_modified) {
            ...
            foreach ($assets['src'] as $key => $asset) {
                ...
                if ($type === 'css') {
                    ...
                    // YUICompressor ?
                    if (self::$enable_yuicompressor && !empty($contents)) {
                        self::_init_yuicompressor();
                        
                        $yui = new YUICompressor\YUI;
                        $yui->addString(trim($contents));
            ...
            if ($type === 'css') {
                ...
                if (self::$minify_css and !self::$freeze) {
                    ...
                    // $output = $ci->yuicompressor->initialize(array('type' => 'css'))->addString($output)->compress();
                }
    ...
    public static function _process_imports($contents = '', $import_dir = null, $asset_location = null) {
        // Start benchmark
        if (self::$_enable_benchmark)
            self::$_ci->benchmark->mark("Assets::process_imports()_start");
        ...
        if ($imports_exist and is_array($imports) and isset($imports[1]) and is_array($imports[1])) {
            foreach ($imports[1] as $import_key => $import) {
                ...
                // Is a directory set
                if ($import_dir)
                    $import_file = $import_dir . '/' . $import_file;
                ...
                if ($import_modified > $last_modified)
                    $last_modified = $import_modified;
                ...
                if ($import_result) {
                    ...
                    // Add to list
                    if ($import_result['file_list'])
                        $import_file_list = array_unique(array_merge($import_file_list, $import_result['file_list']));

                    // Check modified time
                    if ($import_result['last_modified'] > $last_modified)
                        $last_modified = $import_result['last_modified'];
                }
        ...
        // End benchmark
        if (self::$_enable_benchmark)
            self::$_ci->benchmark->mark("Assets::process_imports()_end");
    ...
    private static function _process_urls($contents = null, $asset_location = null) {
        ...
        if (isset($matches[2]) and $matches[2]) {
            if ($asset_location)
                $asset_location = dirname($asset_location) . '/';
            else
                $asset_location = '';
    ...
    public static function last_modified($type = null, $group = null) {
        ...
        // Start benchmark
        if (self::$_enable_benchmark)
            self::$_ci->benchmark->mark("Assets::last_modified(" . $type . ", " . $group . ")_start");
            ...
            if (!is_array($file)) {
                ...
                foreach ($subfiles as $subfile) {
                    if (!is_array($subfile)) {
                        ...
                        if ($subfile_modified > $last_modified)
                            $last_modified = $subfile_modified;
                    }
                }
            }
        }
        ...
        // End benchmark
        if (self::$_enable_benchmark)
            self::$_ci->benchmark->mark("Assets::last_modified(" . $type . ", " . $group . ")_end");
    ...
    private static function _cache_assets($type = null) {
        // Start benchmark
        if (self::$_enable_benchmark)
            self::$_ci->benchmark->mark("Assets::cache_assets()_start");
        ...
        // End benchmark
        if (self::$_enable_benchmark)
            self::$_ci->benchmark->mark("Assets::cache_assets()_end");
    }
    ...
    private static function _get_cache_info() {
        // Start benchmark
        if (self::$_enable_benchmark)
            self::$_ci->benchmark->mark("Assets::_get_cache_info()_start");

        self::$_cache_info = @json_decode(read_file(self::$cache_path . '/' . self::$_cache_info_file));

        // End benchmark
        if (self::$_enable_benchmark)
            self::$_ci->benchmark->mark("Assets::_get_cache_info()_end");
    }
    ...
    public static function _update_cache_info() {
        // Start benchmark
        if (self::$_enable_benchmark)
            self::$_ci->benchmark->mark("Assets::_update_cache_info()_start");
        ...
        // End benchmark
        if (self::$_enable_benchmark)
            self::$_ci->benchmark->mark("Assets::_update_cache_info()_end");
    }
    ...
    private static function _generate_tags($type = null, $echo = true) {
        ...
        if (isset(self::$_assets[$type][$group])) {
            ...
            if ($type === 'css') {
                // Default attributes
                if (!isset($assets_group['attributes']['rel']))
                    $assets_group['attributes']['rel'] = 'stylesheet';
            }
            elseif ($type === 'js') {
                // Default attributes
                if (!isset($assets_group['attributes']['charset']) and !self::$html5)
                    $assets_group['attributes']['charset'] = 'utf-8';
            }
            ...
            // File name
            if (!isset($assets_group['cache_file_name']))
                $assets_group['cache_file_name'] = self::$_cache_info->{$type}->{$group}->cache_file_name;
     ...
     * @return string
     */
    public static function tag($file = null, $type = null, $echo = false, $attributes = '') {
        if ($type === 'css') {
            if (self::$html5)
                $tag = '<link href="' . $file . '"' . $attributes . '>';
            else
                $tag = '<link type="text/css" href="' . $file . '"' . $attributes . ' />';
        }
        elseif ($type === 'js') {
            if (self::$html5)
                $tag = '<script src="' . $file . '"' . $attributes . '></script>';
            else
                $tag = '<script src="' . $file . '" type="text/javascript"' . $attributes . '></script>';
        }
        ...
        if ($files) {
            foreach ($files as $file) {
                if ($group) {
                    ...
                    if ($type === 'css') {
                        if (isset($file_info['extension']) and strtolower($file_info['extension']) === 'css')
                            unlink($file_path);
                    }
                    elseif ($type === 'js') {
                        if (isset($file_info['extension']) and strtolower($file_info['extension']) === 'js')
                            unlink($file_path);
                    }
                    else {
                        if (isset($file_info['extension']) and (strtolower($file_info['extension']) === 'css' or strtolower($file_info['extension']) === 'js'))
                            unlink($file_path);
                        if (isset($file_info['extension']) and strtolower($file_info['extension']) === 'cache')
                            unlink($file_path);
                    }
                }
            }
    ...
    public static function init($cfg = null) {
        ...
        if (!self::$_ci) {
            ...
            // Start benchmark
            if (self::$_enable_benchmark)
                self::$_ci->benchmark->mark("Assets::init()_start");
        ...
        // End benchmark
        if (self::$_enable_benchmark)
            self::$_ci->benchmark->mark("Assets::init()_end");
    }
    ...
    private static function _init_cssmin() {
        if (self::$minify_css and !self::$freeze and !self::$_cssmin_loaded) {
            // Start benchmark
            if (self::$_enable_benchmark)
                self::$_ci->benchmark->mark("Assets::init_cssmin()_start");

            // Load
            if (defined('SPARKPATH'))
                include(reduce_double_slashes(SPARKPATH . 'assets/' . ASSETS_VERSION . '/libraries/cssmin.php'));
            else
                include(reduce_double_slashes(APPPATH . '/third_party/assets/cssmin.php'));
            self::$_cssmin_loaded = true;

            // Set current dir for css min
            if (!isset(self::$cssmin_filters['currentDir']))
                self::$cssmin_filters['currentDir'] = str_replace(site_url(), '', self::$base_url) . '/';

            // End benchmark
            if (self::$_enable_benchmark)
                self::$_ci->benchmark->mark("Assets::init_cssmin()_end");
        }
    }
    ...
    private static function _init_jsmin() {
        if (self::$minify_js and !self::$freeze and !self::$_jsmin_loaded) {
            // Start benchmark
            if (self::$_enable_benchmark)
                self::$_ci->benchmark->mark("Assets::init_jsmin()_start");

            // Load
            if (defined('SPARKPATH'))
                include(reduce_double_slashes(SPARKPATH . 'assets/' . ASSETS_VERSION . '/libraries/jsmin.php'));
            else
                include(reduce_double_slashes(APPPATH . '/third_party/assets/jsmin.php'));
            self::$_jsmin_loaded = true;

            // End benchmark
            if (self::$_enable_benchmark)
                self::$_ci->benchmark->mark("Assets::init_jsmin()_end");
        }
    }
    ...
    private static function _init_less() {
        if (!self::$freeze and !self::$_less_loaded) {
            // Start benchmark
            if (self::$_enable_benchmark)
                self::$_ci->benchmark->mark("Assets::init_less()_start");

            // Load LessPHP
            if (defined('SPARKPATH'))
                include(reduce_double_slashes(SPARKPATH . 'assets/' . ASSETS_VERSION . '/libraries/lessc.php'));
            else
                include(reduce_double_slashes(APPPATH . '/third_party/assets/lessc.php'));
            ...
            // End benchmark
            if (self::$_enable_benchmark)
                self::$_ci->benchmark->mark("Assets::init_less()_end");
        }
    }
    ...
    private static function _init_yuicompressor() {
        if (!self::$_yuicompressor_loaded) {
            // Start benchmark
            if (self::$_enable_benchmark)
                self::$_ci->benchmark->mark("Assets::init_yuicompressor()_start");
            
            $load = reduce_double_slashes(reduce_double_slashes(APPPATH . '/third_party/assets/yuicompressor.php'));
            require_once $load;
            
            self::$_yuicompressor_loaded = true;

            // End benchmark
            if (self::$_enable_benchmark)
                self::$_ci->benchmark->mark("Assets::init_yuicompressor()_end");
        }
    }
    ...
    public static function configure($cfg = null) {
        // Start benchmark
        if (self::$_enable_benchmark)
            self::$_ci->benchmark->mark("Assets::configure()_start");
        ...
        // End benchmark
        if (self::$_enable_benchmark)
            self::$_ci->benchmark->mark("Assets::configure()_end");
    }
