...
class Back_Controller extends CMS_Controller {
   ...
   * @return type
   */
  public function text($dato = "", $name = "", $label = "Texto", $instructios = "Máximo 200 caracteres", $required = true, $span = "span8", $wysiwg = false, $count_text = false, $count = "0", $class_span2 = "span8", $row = 6) {
    $data['title'] = $label;
    $data['name'] = $name;
    $data['instrutions'] = $instructios;
    $data['dato'] = $dato;
          ...
          if (trim($this->_post($key)) !== "") {
            ...
            if ($obj->is_rule($key, "required")) {
              $default = "";
					...
					preg_match("/^imagen/", $key, $d, PREG_OFFSET_CAPTURE);
          //echo $d = strpos($key, 'imagen');
          $i = 0;
          if (!empty($d) or (strtolower($key) == 'imagen_id')) {
            $i = str_replace(array("_id", "imagen"), "", $key);
            $i = (!is_numeric($i)) ? 0 : $i;
